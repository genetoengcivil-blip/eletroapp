...
export function UserDashboard() {
  ...
  const stats = [
    { label: 'Eletropostos', value: totalStations, icon: 'M13 10V3L4 14h7v7l9-11h-7z', color: 'from-blue-500 to-blue-700', link: '/eletropostos' },
    { label: 'Favoritos', value: favoriteCount, icon: 'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z', color: 'from-pink-500 to-rose-600', link: '/dashboard/favorites' },
    { label: 'Avaliações', value: reviewCount, icon: 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z', color: 'from-amber-500 to-orange-600', link: '/dashboard/reviews' },
  ]

  const quickActions = [
    { label: 'Planejar Viagem', desc: 'Calcule sua rota e paradas de recarga', icon: 'M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7', link: '/dashboard/trip-planner', color: 'bg-blue-600 hover:bg-blue-700' },
    { label: 'Marketplace EV', desc: 'Compare carros elétricos', icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4', link: '/dashboard/marketplace', color: 'bg-emerald-600 hover:bg-emerald-700' },
    { label: 'Eletropostos', desc: 'Encontre estações no mapa', icon: 'M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z M15 11a3 3 0 11-6 0 3 3 0 016 0z', link: '/eletropostos', color: 'bg-violet-600 hover:bg-violet-700' },
  ]
          ...
          <div className="mb-8 glass-card p-5">
            ...
            <div className="flex items-center gap-6">
              <div className="text-center">
                <div className="text-3xl font-bold" style={{ color: level.color }}>{loyalty.totalPoints}</div>
                <div className="text-[10px] text-gray-400 font-medium">pontos</div>
              </div>
              <div className="flex-1">
                ...
                <div className="h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-500" style={{ width: `${Math.min(progress, 100)}%`, background: level.color }} />
                </div>
      ...
      {nearbyStations.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Eletropostos Próximos</h2>
            <Link to="/eletropostos" className="text-sm text-blue-600 dark:text-blue-400 font-medium hover:underline">Ver todos</Link>
          </div>
              ...
              <Link key={station.id} to={`/dashboard/station/${station.id}`}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-sm text-gray-900 dark:text-white truncate">{station.name}</h3>
